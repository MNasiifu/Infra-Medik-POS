...
export function useProductStock(filters: ProductStockFilters = {}) {
  const branchId = useAuthStore((s) => s.profile?.branch_id ?? null);
  return useQuery({
    queryKey: [PRODUCT_STOCK_KEY, branchId, filters],
    queryFn: () =>
      inventoryService.getProductsWithAggregatedStock(branchId, filters),
    staleTime: 1000 * 60 * 2,
  });
}
