...
const SALE_TYPE_LABELS: Record<SaleType, string> = {
  walk_in: 'Walk-in', delivery: 'Delivery', account: 'Account',
}
...
export function SalesReportPage() {
  ...
  const columns: GridColDef<SalesReportRow>[] = useMemo(() => [
    {
      field: 'sale_number', headerName: 'Sale #', width: 140,
      renderCell: ({ value, row }: GridRenderCellParams<SalesReportRow>) => (
        <Box py={0.5}>
          ...
          {row.is_voided && <Chip label="Voided" size="small" color="error" sx={{ fontSize: '0.6rem', height: 16 }} />}
        </Box>
      ),
    },
    { field: 'sale_date',    headerName: 'Date',          width: 160, valueFormatter: (v: string) => formatDateTime(v) },
    { field: 'teller_name',  headerName: 'Teller',        flex: 1, minWidth: 130 },
    { field: 'customer_name', headerName: 'Customer',     flex: 1, minWidth: 130,
      renderCell: ({ value }: GridRenderCellParams) => <Typography variant="body2" color={value ? 'text.primary' : 'text.disabled'}>{value ?? '—'}</Typography> },
    { field: 'sale_type', headerName: 'Type', width: 100,
      renderCell: ({ value }: GridRenderCellParams) => (
        <Chip label={SALE_TYPE_LABELS[value as SaleType] ?? value} size="small" variant="outlined" sx={{ borderRadius: '6px', fontSize: '0.7rem' }} />
      ),
    },
    { field: 'items_count', headerName: 'Items', width: 70, align: 'right', headerAlign: 'right' },
    { field: 'subtotal_before_vat', headerName: 'Before VAT', width: 130, align: 'right', headerAlign: 'right',
      renderCell: ({ value }: GridRenderCellParams) => <Typography variant="body2" fontFamily="monospace">{formatUGX(value as number)}</Typography> },
    { field: 'vat_amount', headerName: 'VAT', width: 110, align: 'right', headerAlign: 'right',
      renderCell: ({ value }: GridRenderCellParams) => <Typography variant="body2" fontFamily="monospace">{formatUGX(value as number)}</Typography> },
    { field: 'total_amount', headerName: 'Total', width: 130, align: 'right', headerAlign: 'right',
      renderCell: ({ value }: GridRenderCellParams) => <Typography variant="body2" fontWeight={700} fontFamily="monospace">{formatUGX(value as number)}</Typography> },
    { field: 'payment_methods', headerName: 'Payment', width: 160,
      renderCell: ({ value }: GridRenderCellParams) => (
        <Typography variant="caption">{(value as string).split(', ').map(formatPaymentMethod).join(', ')}</Typography>
      ),
    },
  ], [])

  const totalRevenue  = rows.filter((r) => !r.is_voided).reduce((s, r) => s + r.total_amount, 0)
  const totalVat      = rows.filter((r) => !r.is_voided).reduce((s, r) => s + r.vat_amount, 0)
  ...
  const handleExportExcel = async () => {
    await downloadExcel(
      ...
      [
        ...
        { header: 'Before VAT',    key: 'subtotal_before_vat',  width: 16, numFmt: '#,##0' },
      ...
      { 'Period': `${filters.dateFrom} to ${filters.dateTo}`, 'Generated': new Date().toLocaleString('en-UG') },
    )
  }
  ...
  const handlePrint = () => {
    printReport({
      ...
      columns: [
        ...
        { header: 'Before VAT',   key: 'subtotal_before_vat', align: 'right', render: (v) => formatUGX(v as number) },
        { header: 'VAT',          key: 'vat_amount',          align: 'right', render: (v) => formatUGX(v as number) },
  ...
  return (
    <DashboardTemplate>
      <Box display="flex" alignItems="center" gap={1.5} mb={3}>
        ...
        <Box flex={1}>
          <Typography variant="h5" fontWeight={700}>Sales Report</Typography>
          <Typography variant="body2" color="text.secondary">Full transaction list with VAT and payment breakdown.</Typography>
        </Box>
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, mb: 2.5 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems="flex-end" flexWrap="wrap">
          <TextField label="From" type="date" size="small" value={filters.dateFrom}
            onChange={(e) => setFilters((f) => ({ ...f, dateFrom: e.target.value }))}
            InputLabelProps={{ shrink: true }} sx={{ minWidth: 160 }} />
          <TextField label="To" type="date" size="small" value={filters.dateTo}
          ...
          <Button variant="contained" size="small" startIcon={isLoading ? <CircularProgress size={14} color="inherit" /> : <PlayArrowIcon />}
      ...
      {rows.length > 0 && (
        <Stack direction="row" spacing={3} mb={2} px={0.5}>
          <Box><Typography variant="caption" color="text.secondary">Total revenue</Typography>
            <Typography variant="subtitle1" fontWeight={700} fontFamily="monospace">{formatUGX(totalRevenue)}</Typography></Box>
          <Box><Typography variant="caption" color="text.secondary">VAT collected</Typography>
            <Typography variant="subtitle1" fontWeight={700} fontFamily="monospace">{formatUGX(totalVat)}</Typography></Box>
          <Box><Typography variant="caption" color="text.secondary">Transactions</Typography>
            <Typography variant="subtitle1" fontWeight={700}>{rows.length - voidedCount}</Typography></Box>
          {voidedCount > 0 && (
            <Box><Typography variant="caption" color="text.secondary">Voided</Typography>
              <Typography variant="subtitle1" fontWeight={700} color="error.main">{voidedCount}</Typography></Box>
          )}
        </Stack>
      )}

      {isError && <Alert severity="error" sx={{ mb: 2 }}>Failed to load report. Make sure get_sales_report RPC is deployed.</Alert>}

      {!enabled && !isLoading && (
        <Box py={6} textAlign="center">
          <Typography variant="body2" color="text.secondary">Set your filters and click <strong>Run report</strong> to load data.</Typography>
        </Box>
      )}
